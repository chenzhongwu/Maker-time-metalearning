...
class TrainSubgraphDataset(Dataset):
    def __init__(self, args):
        ...
        self.subgraphs_db = self.env.open_db("train_subgraphs".encode('utf-8'))
    ...
    def get_train_g(self, sup_tri, ent_map_list, ent_mask):
        ...
        return g
    # pattern_tri记录了关系的关系即RPG，关系只有0-3，头尾为关系编号
    def get_pattern_g(self, pattern_tri, rel_map_list, rel_mask):
        ...
        return g
    #
    # def get_time_pattern_g(self, pattern_tri, x_qian_y, x_hou_y, x_tong_y, rel_reidx_list, time_mask):
    #     x_qian_y = torch.LongTensor(x_qian_y)
    #     x_hou_y = torch.LongTensor(x_hou_y)
    #     x_tong_y = torch.LongTensor(x_tong_y)
    #     pattern_tri = torch.LongTensor(pattern_tri)
    #     time_g = dgl.graph((pattern_tri[:, 0].T, pattern_tri[:, 2].T))
    #     time_g.edata['rel'] = pattern_tri[:, 3].T
    #     # g.edata['time'] = triples[:, 3].T
    #
    #     rel_mask_list = np.array(list(map(lambda x: x in time_mask, np.arange(len(rel_reidx_list)))))
    #     rel_map_list = np.array(rel_reidx_list)
    #     rel_map_list[rel_mask_list] = -1
    #
    #     time_g.ndata['ori_idx'] = torch.tensor(rel_map_list)
    #
    #     return time_g
    ...
    def __getitem__(self, idx):
        with self.env.begin(db=self.subgraphs_db) as txn:
            ...
            str_id = '{:08}'.format(idx).encode('utf-8')
            # print(str_id)
            # ent_reidx_list：实体重新编号
            # get函数通过键 查询数据 值,如果要查询的键值没有对应数据，则输出None
            sup_tri, pattern_tri, que_tri, hr2t, rt2h, ent_reidx_list, rel_reidx_list, time_reidx_list = deserialize(txn.get(str_id))

        nentity = len(ent_reidx_list)
        # 从np.delete(np.arange(nentity)移除下标hr2t[(h, r)]对应的数
        que_neg_tail_ent=[]
        for h, r, t, time in que_tri:
            a = np.random.choice(np.delete(np.arange(nentity), hr2t[(h, r, time)]),
                                        self.args.metatrain_num_neg)
            que_neg_tail_ent.append(a)
        # que_neg_tail_ent = [np.random.choice(np.delete(np.arange(nentity), hr2t[(h, r, time)]),
        #                                 self.args.metatrain_num_neg) for h, r, t, time in que_tri]
        que_neg_head_ent = []
        for h, r, t, time in que_tri:
            a = np.random.choice(np.delete(np.arange(nentity), rt2h[(r, t, time)]),
                                 self.args.metatrain_num_neg)
            que_neg_head_ent.append(a)
        # que_neg_head_ent = [np.random.choice(np.delete(np.arange(nentity), rt2h[(r, t, time)]),
        #                                 self.args.metatrain_num_neg) for h, r, t, time  in que_tri]
        ...
        # time_mask = np.random.choice(np.arange(len(time_reidx_list)),
        #                             int(len(time_reidx_list) * random.randint(3, 8) * 0.1), replace=False)

        g = self.get_train_g(sup_tri, ent_reidx_list, ent_mask)
        pattern_g = self.get_pattern_g(pattern_tri, rel_reidx_list, rel_mask)
        # pattern_time_g = self.get_time_pattern_g(pattern_tri, rel_reidx_list, rel_mask)
...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h, r, t, time  = pos_triple
        if self.num_cand == 'all':
            tail_label, head_label = self.get_label(self.hr2t[(h, r, time)], self.rt2h[(r, t, time)])
            pos_triple = torch.LongTensor(pos_triple)

            return pos_triple, tail_label, head_label
        else:
            neg_tail_cand = np.random.choice(np.delete(np.arange(self.num_ent), self.hr2t[(h, r, time)]),
                                             self.num_cand)

            neg_head_cand = np.random.choice(np.delete(np.arange(self.num_ent), self.rt2h[(r, t, time)]),
                                             self.num_cand)
...
class Data(object):
    ...
    def get_pattern_g(self, pattern_tri, rel_reidx_list=None):
        ...
        # 看是否是需要屏蔽的，不需要就直接relebal；需要就按传进来的
    ...
    # def get_time_pattern_g(self, pattern_tris_time, time_reidx_list=None):
    #     triples = torch.LongTensor(pattern_tris_time)
    #     time_g = dgl.graph((triples[:, 0].T, triples[:, 2].T))
    #     time_g.edata['rel'] = triples[:, 1].T
    #     # g.edata['time'] = triples[:, 3].T
    #
    #     # 看是否是需要屏蔽的，不需要就直接relebal；需要就按传进来的
    #     if time_reidx_list is None:
    #         time_g.ndata['ori_idx'] = torch.tensor(np.arange(time_g.num_nodes()))
    #     else:
    #         time_g.ndata['ori_idx'] = torch.tensor(time_reidx_list)
    #
    #     return time_g

    def get_pattern_tri(self, sup_tri):
        sup_tri = torch.LongTensor(sup_tri)
        self.num_time = torch.unique(sup_tri[:, 3]).shape[0]
        ...
        x_qian_y, x_hou_y, x_tong_y = x_qian_y[1:], x_hou_y[1:], x_tong_y[1:]
    ...
    # def get_pattern_tri_time(self, time_h, time_t):
    #     # adjacency matrix for rel and rel of different pattern
    #     # 一乘就是4种元关系下，每种关系之间对应三元关系
    #     h_t_time = torch.matmul(time_h, time_t.T)
    #     t_h_time = torch.matmul(time_t, time_h.T)
    #     t_t_time = torch.matmul(time_t, time_t.T) - torch.diag(torch.sum(time_t, axis=1))
    #     h_h_time = torch.matmul(time_h, time_h.T) - torch.diag(torch.sum(time_h, axis=1))
    #
    ...
    #     p_w = torch.LongTensor([])
    #     # 将4种元关系矩阵转换为链接关系，cat在一起，因为输出是用来构造图的，不用区分元关系0到3
    #     for p_rel_idx, mat in enumerate([h_t_time, t_h_time, t_t_time, h_h_time]):
    #         # print("==============")
    #         # print(mat)
    #         sp_mat = sparse.coo_matrix(mat)
    #         src = torch.cat([src, torch.from_numpy(sp_mat.row)])
    ...
    def get_hr2t_rt2h(self, triples, g):
        hr2t = ddict(list)
        rt2h = ddict(list)
        num_time = len(torch.unique(g.edata['time']))
        num_ent = g.num_nodes()
        time_h = torch.zeros((num_time, num_ent), dtype=torch.int)
        time_t = torch.zeros((num_time, num_ent), dtype=torch.int)
        for tri in triples:
            h, r, t, time  = tri
            hr2t[(h, r, time)].append(t)
            rt2h[(r, t, time)].append(h)
            time_h[time, h] += 1
            time_t[time, t] += 1

        return hr2t, rt2h, time_h, time_t
...
class TrainData(Data):
    def __init__(self, args, data):
        ...
        self.hr2t_train, self.rt2h_train , self.time_h, self.time_t= self.get_hr2t_rt2h(self.train_triples,self.g)
        self.pattern_tri = self.get_pattern_tri(self.train_triples)
        # self.pattern_tris_time = self.get_pattern_tri_time(self.time_h, self.time_t)
        self.pattern_g = self.get_pattern_g(self.pattern_tri).to(args.gpu)
        # self.time_pattern_g = self.get_time_pattern_g(self.pattern_tris_time).to(args.gpu)
...
class ValidData(Data):
    def __init__(self, args, data):
        ...
        self.time_map_list = data['time_map_list']

        # g and pattern g
        self.g = self.get_train_g(self.sup_triples, ent_reidx_list=self.ent_map_list).to(args.gpu)
        self.hr2t_all, self.rt2h_all , self.time_h, self.time_t = self.get_hr2t_rt2h(self.sup_triples + self.que_triples,self.g)
        ...
        # self.time_pattern_g = self.get_time_pattern_g(self.pattern_tris_time, time_reidx_list=self.time_map_list).to(args.gpu)
