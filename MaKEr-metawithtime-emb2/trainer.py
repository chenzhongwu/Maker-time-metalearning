...
class Trainer(object):
    def __init__(self, args):
        ...
        # 每次random walk找到的train中的关系数量都可能不同，但是生成完一个pkl之后，模型就固定下来了
    ...
    def evaluate(self, istest=False, num_cand='all'):
        if not istest:
            ...
            uboth_results, uboth_count = self.get_rank(uboth_dataloader, ent_emb, rel_emb,time_emb, num_cand)
    ...
    def get_rank(self, eval_dataloader, ent_emb, rel_emb,time_emb, num_cand='all'):
        ...
        if num_cand == 'all':
            for batch in eval_dataloader:
                ...
                # print(time_emb)
                # print(pos_triple)
                pred, _ = self.kge_model((pos_triple, None), ent_emb, rel_emb, time_emb ,mode='tail-batch')
                ...
                # head prediction
                pred, _ = self.kge_model((pos_triple, None), ent_emb, rel_emb,time_emb, mode='head-batch')
            ...
            for i in range(self.args.num_sample_cand):
                for batch in eval_dataloader:
                    ...
                    # tail prediction
                    # print("87642389595292")
                    # print(time_emb)
                    # print(pos_triple)
                    pred,_ = self.kge_model((pos_triple, tail_cand), ent_emb, rel_emb,time_emb, mode='tail-batch')
                    tail_ranks = 1 + torch.argsort(torch.argsort(pred, dim=1, descending=True),
                                                   dim=1, descending=False)[b_range, target_idx]
                    # head prediction
                    pred,_ = self.kge_model((pos_triple, head_cand), ent_emb, rel_emb,time_emb, mode='head-batch')
                    head_ranks = 1 + torch.argsort(torch.argsort(pred, dim=1, descending=True),
                                                   dim=1, descending=False)[b_range, target_idx]
